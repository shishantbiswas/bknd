...
These events are emitted by the `Authenticator` class and are available on the `AuthEvents` object.

```typescript
import { AuthEvents } from "bknd";
```

| Event                       | Params                              |  Description                                                             |
| :-------------------------- | :---------------------------------- | :----------------------------------------------------------------------- |
| `AuthBeforeLogin`           | `{ user: User }`                    | Emitted when a user attempts to log in.                                  |
| `AuthAfterLogin`            | `{ user: User }`                    | Emitted when a user successfully logs in.                                |
| `AuthBeforeRegister`        | `{ user: CreateUser }`              | Emitted when a user attempts to register.                                |
| `AuthAfterRegister`         | `{ user: User }`                    | Emitted when a user successfully registers.                              |
| `AuthBeforeLogout`          | `{ user: SafeUser \| undefined }`   | Emitted when a user attempts to log out.                                 |
| `AuthAfterLogout`           | `{ user: SafeUser \| undefined }`   | Emitted when a user successfully logs out.                               |
| `AuthBeforePasswordChange`  | `{ user: User }`                    | Emitted when a user attempts to change their password.                   |
| `AuthAfterPasswordChange`   | `{ user: User }`                    | Emitted when a user successfully changes their password.                 |
